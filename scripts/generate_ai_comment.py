...
- 一般向け6項目に加えて、同じデータを「こども向け」に再構成した6項目も作る。単なる一般向け文章の言い換えにしない。
- こども向けは小学校中〜高学年程度を想定し、「今どうなっているか」が先に分かる短い文にする。
- こども向けでは数字を詰め込みすぎない。最新値や3週の推移など理解に必要な数字だけを残し、差分の細かな数値は省いてよい。
- こども向けでは難しい専門用語を避ける。「前年同期」ではなく「去年の同じころ」など、日常的な言葉を使う。
- こども向けでは、小学校中〜高学年で読みづらい漢字に（ ）で読みがなを付ける。同じ語が同じ文に複数回出る場合は最初の1回だけでよい。
- 地域名には必ず読みがなを付ける。表記例: 新潟（にいがた）、新発田（しばた）、新津（にいつ）、三条（さんじょう）、長岡（ながおか）、魚沼（うおぬま）、南魚沼（みなみうおぬま）、十日町（とおかまち）、柏崎（かしわざき）、糸魚川（いといがわ）、村上（むらかみ）、佐渡（さど）、上越（じょうえつ）。
- こども向けの地域コメントは上位地域名を中心に簡潔にし、細かな人/定点の数値を並べすぎない。
- こども向けの年代コメントは「5～9歳」など具体的な年代を示すが、人/定点の数値は必須ではない。
- こども向けの前年比較は「去年の同じころより多くなっています」など意味を先に伝え、差分値を無理に書かない。
- こども向けの見出しには内容に合う絵文字を1つ付ける。本文には絵文字を乱用しない。
- こども向けでも数値や地域名の事実関係を一般向けから変えない。
- こども向けに新しい医学知識を付け足さない。サイト側で固定表示する「じぶんでできること」と重複する予防・行動助言は生成しない。
- 怖がらせる表現を使わない。
...
出力JSON:
{
  ...
  "kids_headline": "絵文字1つから始める、こども向けの短い見出し",
  "kids_summary": "『いまのようす』として、全県の最新状況を意味優先で短い1〜2文。必要な語には読みがなを付ける",
  "kids_trend": "『増えているの？』に答えるように、最近の増減を短い1〜2文。必要なら直近3週の数字を使う",
  "kids_regional": "『どこで多い？』に答えるように、上位地域名に必ず読みがなを付けて短い1〜2文。細かな数値は並べすぎない",
  "kids_age_group": "『どの年代で多い？』に答えるように、具体的な年代を使って短い1〜2文。人/定点の数値は必須ではない",
  "kids_year_on_year": "『去年とくらべると？』に答えるように、去年の同じころとの違いを意味優先で短い1〜2文。差分値は無理に書かない"
}
""".strip()
