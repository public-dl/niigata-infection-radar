...
def trend_svg(weeks,w=420,h=190):
    ...
    return f'<svg viewBox="0 0 {w} {h}" class="trend-svg"><rect width="{w}" height="{h}" fill="#f8fcfe"/>{grid}<polyline points="{prevpts}" fill="none" stroke="#8fa3b1" stroke-width="2" stroke-dasharray="6 5"/><polyline points="{curpts}" fill="none" stroke="{BLUE}" stroke-width="2.6"/>{dots}{labels}<text x="{left}" y="11" font-size="8" font-weight="700" fill="{BLUE}">今年</text><text x="{left+42}" y="11" font-size="8" fill="#718795">前年同期</text></svg>'
